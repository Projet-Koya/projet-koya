...
const Article = () => {
	...
	useEffect(() => {
		fetch(`http://localhost:3001/art/article/id/${id}`)
			.then((res) => res.json())
			.then((res) => {
				setData(res);
		...
		fetch(`http://localhost:3001/sub/subarticles/articlesid/${id}`)
			...
			.then((res) => {
				...
				console.log(res);
				setSubArticles(res.data.filter((subArticle) => subArticle.articleID._id === id));
				setIsLoading(false);
			});
	}, []);
	...
	return (
        ...
        <div classNameName="flex flex-col mt-10 w-1/2 ">
          ...
          <div classNameName="mt-8 flex justify-center px-10 mx-10 shadow-2xl py-8"><p classNameName="flex justify-center text-color">
			...
			<h1 className="m-10 text-center text-4xl font-extrabold text-gray-900">{subArticles[0].title}</h1>
				...
				<section className="shadow row">
					<div className="tabs">
						{subArticles.map((item) => {
							return (
								<>
									<details className="border-l-2 border-transparent relative flex justify-between items-center p-5 pl-8 pr-8 cursor-pointer select-none tab-label">
										<summary className="">{item.title}</summary>
										<div
											dangerouslySetInnerHTML={{ __html: item.text }}
											className="mt-5 pl-8 pr-8 pb-5 text-grey-darkest"
										></div>
									</details>
									{/* <div className="border-b tab">
											<div className="border-l-2 border-transparent relative">
												...
												<div className="tab-content">
													<div
														dangerouslySetInnerHTML={{ __html: item.text }}
														className="pl-8 pr-8 pb-5 text-grey-darkest"
													></div>
												</div>
											</div>
										</div> */}
